...
def transform(image):
    ...
    return _resize_and_pad(image)


def train_transform(image):
    """
    Augmented transform — use for training only.
    Applies mild rotation, elastic distortion, and brightness/contrast jitter
    BEFORE resizing/padding, so augmentation acts on the real image content,
    not on the padded canvas.
    """
    image = image.convert('L')

    # 1. Slight random rotation (±2.5 degrees) — simulates natural handwriting slant variation.
    ...
    if random.random() < 0.5:  # apply only half the time, to keep some clean samples too
        elastic = T.ElasticTransform(alpha=30.0, sigma=5.0, fill=255)
        image = elastic(image)
